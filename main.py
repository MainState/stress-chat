...
def handle_message():
    ...
    if cheat_command == 'set_turn' and target_turn_value is not None:
        try:
            ...
            error_response = {'error': '올바른 턴 번호를 입력해주세요.'}
            print(f"DEBUG: Backend: Sending error response: {error_response}")
            return jsonify(error_response), 400

    # 일반 메시지 처리
    user_message = data.get('message')
    print(f"DEBUG: Backend: NORMAL message processing branch entered for: {user_message}")
